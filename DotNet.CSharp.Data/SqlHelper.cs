...
namespace DotNet.CSharp.Data
{
    /// <summary>
    /// The SqlHelper class is intended to encapsulate high performance, 
    /// scalable best practices for common uses of SqlClient.
    /// 【翻译：SqlHelper 抽象类旨在封装高性能，可扩展的 SqliClient 常用用法的最佳实践。】
    /// </summary>
    public abstract class SqlHelper
    {
        ...
        /// 【翻译：此方法用于将 SqlParameters 数组附加到 SqlCommand 中。
        /// 此方法将向任何参数分配 DbNull 的值，其方向为
        /// 输入输出和空值。
        /// 此行为将防止使用默认值，但
        /// 这将是比预期的纯输出参数（派生为InputOutput）更不常见的情况。
        /// 用户不提供输入值的地方。】
        /// </summary>
        /// <param name="command">The command to which the parameters will be added【翻译：将添加参数的命令。】</param>
        /// <param name="commandParameters">An array of SqlParameters to be added to command【翻译：要添加到命令中的Sql参数数组。】</param>
        private static void AttachParameters(SqlCommand command, SqlParameter[] commandParameters)
        {
            ...
            if (commandParameters != null)
            {
                foreach (SqlParameter p in commandParameters)
                {
                    if (p != null)
                    {
                        // Check for derived output value with no value assigned【翻译：在没有赋值的情况下检查导出的输出值。】
        ...
        /// 【翻译：此方法将 DATAROW 列值分配给 Sql 参数数组。】
        /// </summary>
        /// <param name="commandParameters">Array of SqlParameters to be assigned values【翻译：要分配值的Sql参数数组。】</param>
        /// <param name="dataRow">The dataRow used to hold the stored procedure's parameter values【翻译：用于保存存储过程的参数值的DATAROW。】</param>
